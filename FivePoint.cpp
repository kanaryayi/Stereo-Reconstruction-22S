...
FivePointExecuter::FivePointExecuter(std::pair<KeyPoints, KeyPoints> setPair, ImagePair sample) {
	std::cout << "FivePointExecuter >> Initialization Start." << std::endl;
	...
	m_sample = sample;
	initPossibleRT();
	std::cout << "FivePointExecuter >> Initialization Completed." << std::endl;
}

std::pair<Rotate, Translate> FivePointExecuter::tryOpenCVPipeline() {
	cv::Mat E, R, t, mask;
...
void FivePointExecuter::initPossibleRT() {
	m_fundamentalMatrix = cv::findFundamentalMat(m_pointSet1, m_pointSet2, cv::FM_8POINT);
	std::cout << "FivePointExecuter >> Fundamental Matrix Found." << std::endl;
	m_essentialMatrix = m_sample.K_img2.t() * m_fundamentalMatrix * m_sample.K_img1;
	std::cout << "FivePointExecuter >> Essential Matrix Calculated." << std::endl;
	...
	m_transformations.push_back(std::make_pair(R2, -T));
	std::cout << "FivePointExecuter >> 4 Possible R and T Loaded." << std::endl;
}

std::vector<std::pair<Rotate, Translate>> FivePointExecuter::getAllPossibleRT() {
	return m_transformations;
}

std::pair<Rotate,Translate> FivePointExecuter::getValidRT() {
	...
	for (std::pair<Rotate, Translate> transform : m_transformations) {
		...
		std::cout << "FivePointExecuter >> Validate Transform " << runnerCounter 
	...
	if (possibleCounter != 1 && realAnswerFound) {
		std::cout << "FivePointExecuter >> Although multiple R and T found, but we choose perfect one." << std::endl;
	} else if (possibleCounter != 1 && !realAnswerFound) {
		perror("FivePointExecuter >> Multiple R and T possible (X).");
		std::cout << std::endl;
	...
	if (!realAnswerFound) {
		std::cout << "FivePointExecuter >> Warning: Bad R and T is used." << std::endl;
	}


	std::cout << "FivePointExecuter >> All Transformations validated." << std::endl;
	std::cout << "FivePointExecuter >> Only Transform " << finalIndex << " will work." << std::endl;

	return std::make_pair(rotation, translation);
}

std::pair<bool, int> FivePointExecuter::isValidRT(std::pair<Rotate, Translate> RTPair, int runnerCounter) {
...
cv::Mat FivePointExecuter::getEssentialMatrix() {
	return m_essentialMatrix;
}

cv::Mat FivePointExecuter::getFundamentalMatrix() {
	return m_fundamentalMatrix;
}

std::pair<cv::Mat, double> FivePointExecuter::getLambdaGamma() {
